...
Scenario: Impossibilidade de Atualizar Entrega em Deslocamento para Solicitada
  ...
  When o usuário clica no botão com texto "Atualizar para Entregue" na primeira entrega existente
  Then o usuário deve clicar em "Voltar" no modal, pois não é possível atualizar uma entrega em deslocamento para solicitada

Scenario: Impossibilidade de Atualizar Entrega Entregue para Deslocamento
  Given o usuário está na página "/historico"
  When o usuário clica no botão com texto "Atualizar para Deslocamento" na primeira entrega existente
  Then o usuário deve clicar em "Voltar" no modal, pois não é possível atualizar uma entrega entregue para deslocamento
